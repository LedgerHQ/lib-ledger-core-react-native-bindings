...
@implementation RCTCoreAddressListCallback
 ...
 */
- (void)onCallback:(nullable NSArray<Address *> *)result
             error:(nullable Error *)error {
    if (error)
    {
        self.reject(@"RCTCoreAddressListCallback Error", error.message, nil);
    }
    ...
    for (id result_elem in result)
    {
        ...
        RCTCoreAddress *rctImpl_result_elem = (RCTCoreAddress *)[self.bridge moduleForName:@"CoreAddress"];
        [rctImpl_result_elem.objcImplementations setObject:result_elem forKey:uuid];
        NSDictionary *converted_result_elem = @{@"type" : @"CoreAddress", @"uid" : uuid };
