...
@implementation RCTCoreOperationListCallback
 ...
 */
- (void)onCallback:(nullable NSArray<Operation *> *)result
             error:(nullable Error *)error {
    if (error)
    {
        self.reject(@"RCTCoreOperationListCallback Error", error.message, nil);
    }
    ...
    for (id result_elem in result)
    {
        ...
        RCTCoreOperation *rctImpl_result_elem = (RCTCoreOperation *)[self.bridge moduleForName:@"CoreOperation"];
        [rctImpl_result_elem.objcImplementations setObject:result_elem forKey:uuid];
        NSDictionary *converted_result_elem = @{@"type" : @"CoreOperation", @"uid" : uuid };
