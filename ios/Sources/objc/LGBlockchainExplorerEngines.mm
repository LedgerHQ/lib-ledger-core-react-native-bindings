// AUTOGENERATED FILE - DO NOT MODIFY!
// This file generated by Djinni from configuration.djinni

#import "LGBlockchainExplorerEngines.h"


NSString * __nonnull const LGBlockchainExplorerEnginesSTARGATENODE = @"STARGATE_NODE";

NSString * __nonnull const LGBlockchainExplorerEnginesCOSMOSNODE = @"COSMOS_NODE";

NSString * __nonnull const LGBlockchainExplorerEnginesLEDGERAPI = @"LEDGER_API";

NSString * __nonnull const LGBlockchainExplorerEnginesRIPPLEAPI = @"RIPPLE_API";

NSString * __nonnull const LGBlockchainExplorerEnginesRIPPLENODE = @"RIPPLE_NODE";

NSString * __nonnull const LGBlockchainExplorerEnginesTEZOSNODE = @"TEZOS_NODE";

NSString * __nonnull const LGBlockchainExplorerEnginesTZSTATSAPI = @"TZSTATS_API";
