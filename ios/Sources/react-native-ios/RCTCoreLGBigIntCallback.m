...
@implementation RCTCoreLGBigIntCallback
...
- (void)onCallback:(nullable LGBigInt *)result
    ...
    if (error)
    {
        self.reject(@"RCTCoreLGBigIntCallback Error", error.message, nil);
        return;
    }

    NSString *result_uuid = [[NSUUID UUID] UUIDString];
    RCTCoreLGBigInt *rctImpl_result = (RCTCoreLGBigInt *)[self.bridge moduleForName:@"CoreLGBigInt"];
    if (result)
    {
        NSArray *result_array = [[NSArray alloc] initWithObjects:result, result_uuid, nil];
        [rctImpl_result baseSetObject:result_array];
    }
    NSDictionary *converted_result = @{@"type" : @"CoreLGBigInt", @"uid" : result_uuid };
