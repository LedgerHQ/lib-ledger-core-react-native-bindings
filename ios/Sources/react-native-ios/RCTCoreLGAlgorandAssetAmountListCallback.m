...
@implementation RCTCoreLGAlgorandAssetAmountListCallback
 ...
 */
- (void)onCallback:(nullable NSArray<LGAlgorandAssetAmount *> *)result
             error:(nullable LGError *)error {
    if (error)
    {
        self.reject(@"RCTCoreLGAlgorandAssetAmountListCallback Error", error.message, nil);
        return;
    }
    ...
    for (id result_elem in result)
    {
        ...
        RCTCoreLGAlgorandAssetAmount *rctImpl_result_elem = (RCTCoreLGAlgorandAssetAmount *)[self.bridge moduleForName:@"CoreLGAlgorandAssetAmount"];
        if (result_elem)
        {
        ...
        NSDictionary *converted_result_elem = @{@"type" : @"CoreLGAlgorandAssetAmount", @"uid" : result_elem_uuid };
