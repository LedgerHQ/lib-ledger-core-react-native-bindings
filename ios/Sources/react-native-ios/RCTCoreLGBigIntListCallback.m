...
@implementation RCTCoreLGBigIntListCallback
 ...
 */
- (void)onCallback:(nullable NSArray<LGBigInt *> *)result
             error:(nullable LGError *)error {
    if (error)
    {
        self.reject(@"RCTCoreLGBigIntListCallback Error", error.message, nil);
        return;
    }
    ...
    for (id result_elem in result)
    {
        ...
        RCTCoreLGBigInt *rctImpl_result_elem = (RCTCoreLGBigInt *)[self.bridge moduleForName:@"CoreLGBigInt"];
        if (result_elem)
        {
        ...
        NSDictionary *converted_result_elem = @{@"type" : @"CoreLGBigInt", @"uid" : result_elem_uuid };
