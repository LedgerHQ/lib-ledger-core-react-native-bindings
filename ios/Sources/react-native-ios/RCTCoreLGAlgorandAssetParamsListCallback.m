...
@implementation RCTCoreLGAlgorandAssetParamsListCallback
 ...
 */
- (void)onCallback:(nullable NSArray<LGAlgorandAssetParams *> *)result
             error:(nullable LGError *)error {
    if (error)
    {
        self.reject(@"RCTCoreLGAlgorandAssetParamsListCallback Error", error.message, nil);
        return;
    }
    ...
    for (id result_elem in result)
    {
        ...
        RCTCoreLGAlgorandAssetParams *rctImpl_result_elem = (RCTCoreLGAlgorandAssetParams *)[self.bridge moduleForName:@"CoreLGAlgorandAssetParams"];
        if (result_elem)
        {
        ...
        NSDictionary *converted_result_elem = @{@"type" : @"CoreLGAlgorandAssetParams", @"uid" : result_elem_uuid };
