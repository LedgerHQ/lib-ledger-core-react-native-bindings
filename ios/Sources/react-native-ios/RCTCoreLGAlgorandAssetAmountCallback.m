...
@implementation RCTCoreLGAlgorandAssetAmountCallback
...
- (void)onCallback:(nullable LGAlgorandAssetAmount *)result
    ...
    if (error)
    {
        self.reject(@"RCTCoreLGAlgorandAssetAmountCallback Error", error.message, nil);
        return;
    }

    NSString *result_uuid = [[NSUUID UUID] UUIDString];
    RCTCoreLGAlgorandAssetAmount *rctImpl_result = (RCTCoreLGAlgorandAssetAmount *)[self.bridge moduleForName:@"CoreLGAlgorandAssetAmount"];
    if (result)
    {
        NSArray *result_array = [[NSArray alloc] initWithObjects:result, result_uuid, nil];
        [rctImpl_result baseSetObject:result_array];
    }
    NSDictionary *converted_result = @{@"type" : @"CoreLGAlgorandAssetAmount", @"uid" : result_uuid };
