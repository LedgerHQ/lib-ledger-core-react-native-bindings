...
@implementation RCTCoreLGStellarLikeFeeStatsCallback
...
- (void)onCallback:(nullable LGStellarLikeFeeStats *)result
    ...
    if (error)
    {
        self.reject(@"RCTCoreLGStellarLikeFeeStatsCallback Error", error.message, nil);
        return;
    }

    NSString *result_uuid = [[NSUUID UUID] UUIDString];
    RCTCoreLGStellarLikeFeeStats *rctImpl_result = (RCTCoreLGStellarLikeFeeStats *)[self.bridge moduleForName:@"CoreLGStellarLikeFeeStats"];
    if (result)
    {
        NSArray *result_array = [[NSArray alloc] initWithObjects:result, result_uuid, nil];
        [rctImpl_result baseSetObject:result_array];
    }
    NSDictionary *converted_result = @{@"type" : @"CoreLGStellarLikeFeeStats", @"uid" : result_uuid };
